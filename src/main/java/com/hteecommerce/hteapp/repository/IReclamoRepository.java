...
public interface IReclamoRepository extends JpaRepository<Reclamo,Integer> {
    ...
    @Query("select re from Reclamo re join re.cliente cli where cli.idcliente = ?1 order by re.idreclamo desc")
    public List<Reclamo> listByIdcliente(Integer idcliente);

    @Query("select re from Reclamo re join re.proveedor pro where pro.idproveedor = ?1 order by re.idreclamo desc")
    public List<Reclamo> listByIdproveedor(Integer idproveedor);

    public List<Reclamo> findByFecha(LocalDate fecha);

    @Query("select re from Reclamo re order by re.idreclamo desc")

...
public interface IDetalleIngresoRepository extends JpaRepository<DetalleIngreso, Integer> {   
    ...
    @Query("from DetalleIngreso di join di.producto pro where pro.codigo like %?1% or upper(trim(pro.nombre)) like concat('%',upper(?2),'%')")
    ...
    @Query("from DetalleIngreso di order by di.iddetalleingreso desc")
    public Page<DetalleIngreso> pageAll(Pageable pageable);

    @Query("from DetalleIngreso di join di.producto pro where pro.idproducto = ?1")
